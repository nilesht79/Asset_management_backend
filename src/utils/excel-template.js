...
vendors.forEach(v => {
  ...
  worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
    ...
    if (rowData.serial_number) {
      ...
      if (seenSerialNumbers.has(String(rowData.serial_number).toLowerCase()))
      {
        errors.push('Duplicate serial number within file');
      } else {
        // seenSerialNumbers.add(rowData.serial_number.toLowerCase());
        seenSerialNumbers.add(String(rowData.serial_number).toLowerCase());
      }
    }
    ...
    // }
     ...
     if (rowData.serial_number && existingSerialNumbers.includes(String(rowData.serial_number).toLowerCase()) 
     {
      errors.push('Serial number already exists in database');
    }
